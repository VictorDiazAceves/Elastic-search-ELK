package com.PoC.ElkSpringBoot.ELK.SpringBoot;

import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RestController;

import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;

@RestController
@Slf4j
public class HomeController {



    @GetMapping("/debug")
    public String debugPage(){
        LocalDateTime localDateTime = LocalDateTime.now();
        log.debug("This debug page " + localDateTime);
        return "Welcome to debug page ";
    }

    @GetMapping("/logs")
    public String LogsPage(){
        LocalDateTime localDateTime = LocalDateTime.now();
        log.info("This Logs page " + localDateTime);
        return "Welcome to logs page";
    }

    @GetMapping("/warn")
    public String WarnPage(){
        LocalDateTime localDateTime = LocalDateTime.now();
        log.warn("This warn page " + localDateTime);
        return "Welcome to warn page";
    }


    @GetMapping("/er")
    public String ErrorPage(){
        LocalDateTime localDateTime = LocalDateTime.now();
        log.error("This error page " + localDateTime);
        return "Welcome to error page ";
    }

    @GetMapping("/tr")
    public String tracePage(){
        LocalDateTime localDateTime = LocalDateTime.now();
        log.trace("This trace page " + localDateTime);
        return "Welcome to trace page ";
    }

}

